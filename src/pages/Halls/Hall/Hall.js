...
export default {
    ...
    methods: {
        ...
        loadData: function () {
            ...
            this.$http
                .GetHallById(this.$route.params.id)
                .then(response => {
                    ...
                    if (response.status == 200) {
                        ...
                        this.loaded = 204;
                        this.$alert.Empty("هذا العنصر غير متوفرة");
                    }
                    else {
                        this.loaded = 400;
                    }
       ...
       deleteHall: function (id) {
            Swal.fire({
                title: "هل أنت متأكد",
                text: "هل أنت متأكد من أنك تريد حذف هذه القاعة !",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    /*this.$loading.Start();*/ this.$store.commit("loadingStart");
                    this.$http
                        .DeleteHall(this.$route.params.id)
                        .then(response => {
                            /*this.$loading.Stop();*/ this.$store.commit("loadingStop");
                            if (response.status == 200) {
                                this.mainItem = [];
                                ...
                                this.loaded = 204;
                                this.$alert.Empty(
                                    "لم يعد هذا العنصر متوفرا, قد يكون شخص أخر قام بحذفه"
                                );
                            }
                        })
                        .catch(error => {
        ...
        Swal.fire({
            ...
            if(result.isConfirmed) {
                ...
                this.$http.ActiveHall(this.$route.params.id)
                    .then(response => {
        ...
        Swal.fire({
            title: "هل أنت متأكد؟",
            text: "هل تريد إلغاء تفعيل هذه القاعة؟",
            icon: "warning",
            showCancelButton: true,
            ...
            if(result.isConfirmed) {
                this.$store.commit("loadingStart");
                this.$http.DisActiveHall(this.$route.params.id)
                    .then(response => {
                        this.$store.commit("loadingStop");
                        if(response.status == 200) {
