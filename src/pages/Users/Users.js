...
export default {
    ...
    data() {
        return {
            mainList: [],
            cities: [], // List of cities for the filter
    ...
    methods: {
        ...
        loadCities: function () {
            this.$http.GetAllCities(1, 100, "") // Fetch a reasonable number of cities
                .then(response => {
        ...
        loadData: function (page) {
            ...
            this.$http
                .GetAllUsers(this.pageId, this.countPerPage, this.tagId, this.phoneSrh, this.firstNameSrh, this.lastNameSrh, this.cityIdSrh)
                .then(response => {
            ...
            /*this.$loading.Start();*/ this.$store.commit("loadingStart");
                    this.$http
                        ...
                        .then(response => {
                            /*this.$loading.Stop();*/ this.$store.commit("loadingStop");
                            if (response.status == 200) {
                                this.mainList[index].status = 2; // Active status is 2 based on API docs
                ...
                if (result.isConfirmed) {
                    ...
                    this.$http
                        ...
                        .then(response => {
                            ...
                            if (response.status == 200) {
                                this.mainList[index].status = 1; // Inactive status is 1 based on API docs
                                this.$alert.Success(response.data.message);
                            } else if (response.status == 204) {
                                this.$alert.Empty(
                                    "لم يعد هذا المستخدم متوفر, قد يكون شخص أخر قام بحذفه"
                ...
                if (result.isConfirmed) {
                    ...
                    this.$http
                        ...
                        .then(response => {
                            this.$store.commit("loadingStop");
                            if (response.status == 200) {
                                this.mainList[index].status = 2; // Return to active status (2)
                                this.$alert.Success(response.data.message);
                            } else if (response.status == 204) {
                                this.$alert.Empty(
                                    "لم يعد هذا المستخدم متوفر, قد يكون شخص أخر قام بحذفه"
