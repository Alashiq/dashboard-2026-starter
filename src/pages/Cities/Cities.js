...
export default {
    ...
    methods: {
        ...
        loadData: function (page) {
            ...
            this.$http
                .GetAllCities(this.pageId, this.countPerPage, this.nameSrh)
                .then(response => {
            ...
            /*this.$loading.Start();*/ this.$store.commit("loadingStart");
                    this.$http
                        ...
                        .then(response => {
                            /*this.$loading.Stop();*/ this.$store.commit("loadingStop");
                            if (response.status == 200) {
                                this.mainList[this.mainList.findIndex(m => m.id === id)].status = 'active';
                                this.$alert.Success(response.data.message);
                            } else if (response.status == 204) {
                                this.$alert.Empty(
                ...
                if (result.isConfirmed) {
                    ...
                    this.$http
                        ...
                        .then(response => {
                            /*this.$loading.Stop();*/ this.$store.commit("loadingStop");
                            if (response.status == 200) {
                                this.mainList[this.mainList.findIndex(m => m.id === id)].status = 'not_active';
                                this.$alert.Success(response.data.message);
                            } else if (response.status == 204) {
                                this.$alert.Empty(
                                    "لم يعد هذا الحساب متوفرة, قد يكون شخص أخر قام بحذفه"
        ...
        deleteCity: function (id) {
            Swal.fire({
                title: "هل أنت متأكد",
                text: "هل أنت متأكد من أنك تريد حذف هذه المدينة !",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#16a085",
                cancelButtonColor: "#d33",
                    ...
                    this.$http
                        ...
                        .then(response => {
                            ...
                            if (response.status == 200) {
                                ...
                                this.mainList.splice(this.mainList.findIndex(m => m.id === id), 1);
                                this.$alert.Empty(
                                    "لم تعد هذه المدينة متوفرة, قد يكون شخص أخر قام بحذفها"
                                );
                            }
                        })
                        .catch(error => {
